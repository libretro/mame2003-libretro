...
/* Defined in driver */
extern uint16_t *mcatadv_videoram1, *mcatadv_videoram2;
extern uint16_t *mcatadv_scroll, *mcatadv_scroll2;
extern uint16_t *mcatadv_vidregs;

static struct tilemap *mcatadv_tilemap1,  *mcatadv_tilemap2;
static uint16_t *spriteram_old, *vidregs_old;
...
static void mcatadv_drawsprites ( struct mame_bitmap *bitmap, const struct rectangle *cliprect )
{
	...
	while ( source<finish )
	{
		...
		uint8_t *sprdata = memory_region ( REGION_GFX1 );
