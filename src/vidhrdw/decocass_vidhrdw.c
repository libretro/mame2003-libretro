...
static uint32_t tile_offset[32*32] = {
...
static uint32_t fgvideoram_scan_cols( uint32_t col, uint32_t row, uint32_t num_cols, uint32_t num_rows )
{
	/* logical (col,row) -> memory offset */
	return (num_cols - 1 - col) * num_rows + row;
}

static uint32_t bgvideoram_scan_cols( uint32_t col, uint32_t row, uint32_t num_cols, uint32_t num_rows )
{
	/* logical (col,row) -> memory offset */
