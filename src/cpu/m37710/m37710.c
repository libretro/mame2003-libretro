...
#include "m37710_compat.h"
#include "m37710.h"
#include "m37710cm.h"

#define M37710_DEBUG	(0)	/* enables verbose logging for peripherals, etc. */

static void m37710_intl_set_irq_line(m37710i_cpu_struct *cpustate, int line, int state);
...
{
	m37710i_cpu_struct *cpustate = (m37710i_cpu_struct *)m37710_get_token();
	int which = param;
	...
	m37710_intl_set_irq_line(cpustate, curirq, PULSE_LINE);
	cpu_triggerint(cpustate->cpunum);
}
...
static void m37710_recalc_timer(m37710i_cpu_struct *cpustate, int timer)
{
	...
	if (cpustate->m37710_regs[0x40] & (1<<timer))
	{
		...
		if (timer < 5)
		{
			...
			{
				case 0:	    	/* timer mode */
					time = attotime_mul(ATTOTIME_IN_HZ(cpustate->clock), tscales[cpustate->m37710_regs[tcr[timer]]>>6]);
...
#if 0  /* device internal memory map is provided by the namco driver */
...
/*              logerror("line %d set, level %x curpri %x IPL %x\n", curirq, cpustate->m37710_regs[m37710_irq_levels[curirq]] & 7, curpri, cpustate->ipl); */
	...
	if (wantedIRQ != -1)
	{
		...
		if (wantedIRQ <= 12)
		{
			m37710_intl_set_irq_line(cpustate, wantedIRQ, CLEAR_LINE);
		}
...
/* Get a register */
static unsigned m37710_intl_get_reg(m37710i_cpu_struct *cpustate, int regnum)
{
	return FTABLE_GET_REG(cpustate, regnum);
}

/* Set a register */
static void m37710_intl_set_reg(m37710i_cpu_struct *cpustate, int regnum, unsigned value)
{
...
static CPU_INIT( m37710 )
{
	...
	cpustate->program = &m37710_space_program;
	cpustate->io = &m37710_space_io;
	...
	for (i = 0; i < 8; i++)
		cpustate->timers[i] = timer_alloc(m37710_timer_cb);
...
#if 0  /* mame2003 uses explicit interface functions (see glue below) */
static CPU_SET_INFO( m37710 )
{
	...
	switch (state)
	{
		...
		case CPUINFO_INT_INPUT_STATE + M37710_LINE_ADC: 	m37710_intl_set_irq_line(cpustate, M37710_LINE_ADC, info->i); break;
		case CPUINFO_INT_INPUT_STATE + M37710_LINE_IRQ0:	m37710_intl_set_irq_line(cpustate, M37710_LINE_IRQ0, info->i); break;
		case CPUINFO_INT_INPUT_STATE + M37710_LINE_IRQ1:	m37710_intl_set_irq_line(cpustate, M37710_LINE_IRQ1, info->i); break;
		case CPUINFO_INT_INPUT_STATE + M37710_LINE_IRQ2:	m37710_intl_set_irq_line(cpustate, M37710_LINE_IRQ2, info->i); break;
		...
		case CPUINFO_INT_REGISTER + M37710_PC:			m37710_intl_set_reg(cpustate, M37710_PC, info->i);		break;
		case CPUINFO_INT_REGISTER + M37710_S:			m37710_intl_set_reg(cpustate, M37710_S, info->i);		break;
		case CPUINFO_INT_REGISTER + M37710_P:			m37710_intl_set_reg(cpustate, M37710_P, info->i&0xff);	cpustate->ipl = (info->i>>8)&0xff;	break;
		case CPUINFO_INT_REGISTER + M37710_A:			m37710_intl_set_reg(cpustate, M37710_A, info->i);		break;
		case CPUINFO_INT_REGISTER + M37710_X:			m37710_intl_set_reg(cpustate, M37710_X, info->i);		break;
		case CPUINFO_INT_REGISTER + M37710_Y:			m37710_intl_set_reg(cpustate, M37710_Y, info->i);		break;
		case CPUINFO_INT_REGISTER + M37710_PB:			m37710_intl_set_reg(cpustate, M37710_PB, info->i);		break;
		case CPUINFO_INT_REGISTER + M37710_DB:			m37710_intl_set_reg(cpustate, M37710_DB, info->i);		break;
		case CPUINFO_INT_REGISTER + M37710_D:			m37710_intl_set_reg(cpustate, M37710_D, info->i);		break;
		case CPUINFO_INT_REGISTER + M37710_E:			m37710_intl_set_reg(cpustate, M37710_E, info->i);		break;
		case CPUINFO_INT_REGISTER + M37710_NMI_STATE:	m37710_intl_set_reg(cpustate, M37710_NMI_STATE, info->i); break;
		case CPUINFO_INT_REGISTER + M37710_IRQ_STATE:	m37710_intl_set_reg(cpustate, M37710_IRQ_STATE, info->i); break;
	}
}
...
CPU_GET_INFO( m37710 )
{
	...
	switch (state)
	{
		...
		case CPUINFO_INT_REGISTER + M37710_PC:			info->i = m37710_intl_get_reg(cpustate, M37710_PC);	break;
		case CPUINFO_INT_REGISTER + M37710_S:			info->i = m37710_intl_get_reg(cpustate, M37710_S);		break;
		case CPUINFO_INT_REGISTER + M37710_P:			info->i = m37710_intl_get_reg(cpustate, M37710_P) | (cpustate->ipl<<8); break;
		case CPUINFO_INT_REGISTER + M37710_A:			info->i = m37710_intl_get_reg(cpustate, M37710_A);		break;
		case CPUINFO_INT_REGISTER + M37710_B:			info->i = m37710_intl_get_reg(cpustate, M37710_B);		break;
		case CPUINFO_INT_REGISTER + M37710_X:			info->i = m37710_intl_get_reg(cpustate, M37710_X);		break;
		case CPUINFO_INT_REGISTER + M37710_Y:			info->i = m37710_intl_get_reg(cpustate, M37710_Y);		break;
		case CPUINFO_INT_REGISTER + M37710_PB:			info->i = m37710_intl_get_reg(cpustate, M37710_PB);	break;
		case CPUINFO_INT_REGISTER + M37710_DB:			info->i = m37710_intl_get_reg(cpustate, M37710_DB);	break;
		case CPUINFO_INT_REGISTER + M37710_D:			info->i = m37710_intl_get_reg(cpustate, M37710_D);		break;
		case CPUINFO_INT_REGISTER + M37710_E:			info->i = m37710_intl_get_reg(cpustate, M37710_E);		break;
		case CPUINFO_INT_REGISTER + M37710_NMI_STATE:	info->i = m37710_intl_get_reg(cpustate, M37710_NMI_STATE); break;
		case CPUINFO_INT_REGISTER + M37710_IRQ_STATE:	info->i = m37710_intl_get_reg(cpustate, M37710_IRQ_STATE); break;
...
/* ======================================================================== */
/* === mame2003 cpu_interface glue ======================================== */
/* ======================================================================== */

/* System 11 carries a single C76, so the device collapses to one global
   instance; mame2003 swaps it against the main CPU via get/set_context. */
static m37710i_cpu_struct m37710_global;

/* Address-space tags handed to the byte accessors (see m37710_compat.h). */
const address_space m37710_space_program = M37710_AS_PROGRAM;
const address_space m37710_space_io      = M37710_AS_IO;

/* Cycle counter referenced by the CPU0() registration in cpuintrf.c. */
int m37710_ICount = 0;
...
{
	(void)param;
	cpu_reset_m37710((legacy_cpu_device *)0);
}

void m37710_exit(void)
{
	cpu_exit_m37710((legacy_cpu_device *)0);
}

int m37710_execute(int cycles)
{
	m37710_global.ICount = cycles;
	cpu_execute_m37710((legacy_cpu_device *)0);
	m37710_ICount = m37710_global.ICount;
	return cycles - m37710_global.ICount;
}

unsigned m37710_get_context(void *dst)
{
	if (dst)
		*(m37710i_cpu_struct *)dst = m37710_global;
	return sizeof(m37710i_cpu_struct);
}

void m37710_set_context(void *src)
{
	if (src)
		m37710_global = *(m37710i_cpu_struct *)src;
}

unsigned m37710_get_reg(int regnum)
{
	return m37710_intl_get_reg(&m37710_global, regnum);
}

void m37710_set_reg(int regnum, unsigned val)
{
	m37710_intl_set_reg(&m37710_global, regnum, val);
}

void m37710_set_irq_line(int irqline, int state)
{
	m37710_intl_set_irq_line(&m37710_global, irqline, state);
}

void m37710_set_irq_callback(int (*callback)(int irqline))
{
	m37710_global.int_ack = callback;
}
...
const char *m37710_info(void *context, int regnum)
{
	...
	switch (regnum)
	{
		case CPU_INFO_NAME:       return "M37710";
		case CPU_INFO_FAMILY:     return "Mitsubishi M377xx";
		case CPU_INFO_VERSION:    return "1.0";
		case CPU_INFO_FILE:       return __FILE__;
		case CPU_INFO_CREDITS:    return "Copyright the MAME team";
		case CPU_INFO_REG_LAYOUT: return (const char *)m37710_reg_layout;
		case CPU_INFO_WIN_LAYOUT: return (const char *)m37710_win_layout;
	}
	return "";
}
...
unsigned m37710_dasm(char *buffer, unsigned pc)
{
	...
	return (unsigned)cpu_disassemble_m37710((legacy_cpu_device *)0, buffer, pc, oprom, oprom, 0);
}
