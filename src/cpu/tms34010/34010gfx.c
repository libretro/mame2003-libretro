...
static void shiftreg_w(uint32_t offset,uint16_t data)
{
...
static uint16_t shiftreg_r(uint32_t offset)
{
...
static uint16_t dummy_shiftreg_r(uint32_t offset)
{
	return state.shiftreg[0];
}
...
static void FUNCTION_NAME(pixblt)(int src_is_linear, int dst_is_linear)
{
	...
	if (!P_FLAG)
	{
		...
		void (*word_write)(uint32_t address,uint16_t data);
