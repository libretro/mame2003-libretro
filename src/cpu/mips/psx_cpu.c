		...
		case OP_LWL:
			...
			else if( ( mipscpu.cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				else
				{
					...
					{
					case 0:
						n_res = ( mipscpu.r[ INS_RT( mipscpu.op ) ] & 0x00ffffff ) | ( (uint32_t)cpu_readmem32ledw( n_adr + 3 ) << 24 );
						break;
					case 1:
						n_res = ( mipscpu.r[ INS_RT( mipscpu.op ) ] & 0x0000ffff ) | ( (uint32_t)cpu_readmem32ledw_word( n_adr + 1 ) << 16 );
						break;
					case 2:
						n_res = ( mipscpu.r[ INS_RT( mipscpu.op ) ] & 0x000000ff ) | ( (uint32_t)cpu_readmem32ledw( n_adr - 1 ) << 8 ) | ( (uint32_t)cpu_readmem32ledw_word( n_adr ) << 16 );
						break;
					default:
			...
			else
			{
				...
				else
				{
					...
					{
					case 0:
						n_res = ( mipscpu.r[ INS_RT( mipscpu.op ) ] & 0x00ffffff ) | ( (uint32_t)cpu_readmem32ledw( n_adr ) << 24 );
						break;
					case 1:
						n_res = ( mipscpu.r[ INS_RT( mipscpu.op ) ] & 0x0000ffff ) | ( (uint32_t)cpu_readmem32ledw_word( n_adr - 1 ) << 16 );
						break;
					case 2:
						n_res = ( mipscpu.r[ INS_RT( mipscpu.op ) ] & 0x000000ff ) | ( (uint32_t)cpu_readmem32ledw_word( n_adr - 2 ) << 8 ) | ( (uint32_t)cpu_readmem32ledw( n_adr ) << 24 );
						break;
					default:
						n_res = cpu_readmem32ledw_dword( n_adr - 3 );
						break;
			...
			else if( ( mipscpu.cp0r[ CP0_SR ] & ( SR_RE | SR_KUC ) ) == ( SR_RE | SR_KUC ) )
			{
				...
				else
				{
					...
					case 2:
						...
						break;
					case 1:
						n_res = ( mipscpu.r[ INS_RT( mipscpu.op ) ] & 0xff000000 ) | cpu_readmem32ledw_word( n_adr - 1 ) | ( (uint32_t)cpu_readmem32ledw( n_adr + 1 ) << 16 );
						break;
					default:
			...
			else
			{
				...
				else
				{
					...
					case 2:
						...
						break;
					case 1:
						n_res = ( mipscpu.r[ INS_RT( mipscpu.op ) ] & 0xff000000 ) | cpu_readmem32ledw( n_adr ) | ( (uint32_t)cpu_readmem32ledw_word( n_adr + 1 ) << 8 );
						break;
					default:
						n_res = cpu_readmem32ledw_dword( n_adr );
						break;
...
static void docop2( int gteop )
{
	...
	case 0x1a:
		if( GTE_CT( gteop ) == 0x03e )
		{
			...
			MAC1 = A1( ( ( (int64_t)(int32_t)MAC1 << 12 ) + ( (int16_t)IR0 * (int16_t)IR1 ) ) >> n_sf );
			MAC2 = A2( ( ( (int64_t)(int32_t)MAC2 << 12 ) + ( (int16_t)IR0 * (int16_t)IR2 ) ) >> n_sf );
			MAC3 = A3( ( ( (int64_t)(int32_t)MAC3 << 12 ) + ( (int16_t)IR0 * (int16_t)IR3 ) ) >> n_sf );
			IR1 = Lm_B1( (int32_t)MAC1, 0 );
			IR2 = Lm_B2( (int32_t)MAC2, 0 );
			IR3 = Lm_B3( (int32_t)MAC3, 0 );
			CD0 = CD1;
