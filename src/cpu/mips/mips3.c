...
#define RSVAL32		((uint32_t)mips3.r[RSREG])
#define RTVAL32		((uint32_t)mips3.r[RTREG])
#define RDVAL32		((uint32_t)mips3.r[RDREG])
...
#define HIVAL		(uint32_t)mips3.hi
#define LOVAL		(uint32_t)mips3.lo
...
static INLINE uint64_t get_cop0_reg(int idx)
{
	if (idx == COP0_Count)
	{
		...
		if (mips3_icount >= 25)
			...
			mips3_icount = 0;
		return (uint32_t)((activecpu_gettotalcycles64() - mips3.count_zero_time) / 2);
	}
...
static INLINE void set_cop0_reg(int idx, uint64_t val)
{
	switch (idx)
	{
		case COP0_Cause:
		...
		case COP0_Status:
		{
			...
			uint32_t diff = mips3.cpr[0][idx] ^ val;
			mips3.cpr[0][idx] = val;
			if (diff & 0x8000)
		...
		case COP0_Count:
			mips3.count_zero_time = activecpu_gettotalcycles64() - ((uint64_t)val * 2);
			update_cycle_counting();
			break;
...
static INLINE uint64_t get_cop0_creg(int idx)
{
	return mips3.ccr[0][idx];
}

static INLINE void set_cop0_creg(int idx, uint64_t val)
{
	mips3.ccr[0][idx] = val;
}
...
static INLINE uint64_t get_cop1_reg(int idx)
{
	return mips3.cpr[1][idx];
}

static INLINE void set_cop1_reg(int idx, uint64_t val)
{
	mips3.cpr[1][idx] = val;
}

static INLINE uint64_t get_cop1_creg(int idx)
{
	return mips3.ccr[1][idx];
}

static INLINE void set_cop1_creg(int idx, uint64_t val)
{
	mips3.ccr[1][idx] = val;
}
...
static INLINE void handle_cop1(uint32_t op)
{
	...
	switch (RSREG)
	{
		case 0x00:	/* MFCz */		if (RTREG) RTVAL64 = (int32_t)get_cop1_reg(RDREG);		break;
			...
			switch (op & 0x3f)
			{
				...
				case 0x08:
					if (IS_SINGLE(op))	/* ROUND.L.S */
					{
						...
						if (temp < 0)
							...
							temp = floor(temp + 0.5);
						mips3.cpr[1][FDREG] = (int64_t)temp;
					}
					else				/* ROUND.L.D */
					{
						double temp = FSVALD;
				...
				case 0x0a:
					if (IS_SINGLE(op))	/* CEIL.L.S */
						...
						dtemp = ceil(FSVALD);
					mips3.cpr[1][FDREG] = (int64_t)dtemp;
					break;
				...
				case 0x0c:
					if (IS_SINGLE(op))	/* ROUND.W.S */
					{
						...
						mips3.cpr[1][FDREG] = (int32_t)dtemp;
					}
					else				/* ROUND.W.D */
					{
						dtemp = FSVALD;
				...
				case 0x20:
					if (IS_INTEGRAL(op))
					{
						if (IS_SINGLE(op))	/* CVT.S.W */
							FDVALS = (int32_t)mips3.cpr[1][FSREG];
						else				/* CVT.S.L */
							FDVALS = (int64_t)mips3.cpr[1][FSREG];
					}
					else					/* CVT.S.D */
						FDVALS = FSVALD;
					break;
				...
				case 0x21:
					if (IS_INTEGRAL(op))
					{
						if (IS_SINGLE(op))	/* CVT.D.W */
							FDVALD = (int32_t)mips3.cpr[1][FSREG];
						else				/* CVT.D.L */
							FDVALD = (int64_t)mips3.cpr[1][FSREG];
					}
					else					/* CVT.D.S */
						FDVALD = FSVALS;
					break;
				...
				case 0x24:
					if (IS_SINGLE(op))	/* CVT.W.S */
						mips3.cpr[1][FDREG] = (int32_t)FSVALS;
					else
						mips3.cpr[1][FDREG] = (int32_t)FSVALD;
					break;

				case 0x25:
					if (IS_SINGLE(op))	/* CVT.L.S */
						mips3.cpr[1][FDREG] = (int64_t)FSVALS;
					else				/* CVT.L.D */
						mips3.cpr[1][FDREG] = (int64_t)FSVALD;
					break;
...
static INLINE void handle_cop1x(uint32_t op)
{
	if (!(SR & SR_COP1))
		generate_exception(EXCEPTION_BADCOP, 1);
...
static INLINE uint64_t get_cop2_reg(int idx)
{
	return mips3.cpr[2][idx];
}

static INLINE void set_cop2_reg(int idx, uint64_t val)
{
	mips3.cpr[2][idx] = val;
}

static INLINE uint64_t get_cop2_creg(int idx)
{
	return mips3.ccr[2][idx];
}

static INLINE void set_cop2_creg(int idx, uint64_t val)
{
	mips3.ccr[2][idx] = val;
}
		...
		switch (op >> 26)
		{
			case 0x00:	/* SPECIAL */
				switch (op & 63)
				{
					...
					case 0x20:	/* ADD */
						...
						else RDVAL64 = (int32_t)(RSVAL32 + RTVAL32);
						break;
			...
			case 0x01:	/* REGIMM */
				switch (RTREG)
				{
					...
					case 0x02:	/* BLTZL */		if ((int64_t)RSVAL64 < 0) ADDPC(SIMMVAL);	else mips3.pc += 4;		break;
					case 0x03:	/* BGEZL */		if ((int64_t)RSVAL64 >= 0) ADDPC(SIMMVAL); else mips3.pc += 4; 	break;
					...
					case 0x12:	/* BLTZALL */	if ((int64_t)RSVAL64 < 0) ADDPCL(SIMMVAL,31) else mips3.pc += 4;	break;
					case 0x13:	/* BGEZALL */	if ((int64_t)RSVAL64 >= 0) ADDPCL(SIMMVAL,31) else mips3.pc += 4;	break;
					default:	/* ??? */		invalid_instruction(op);										break;
				}
				break;
			...
			case 0x16:	/* BLEZL */		if ((int64_t)RSVAL64 <= 0) ADDPC(SIMMVAL); else mips3.pc += 4;			break;
			case 0x17:	/* BGTZL */		if ((int64_t)RSVAL64 > 0) ADDPC(SIMMVAL); else mips3.pc += 4;				break;
			case 0x18:	/* DADDI */
			...
			case 0x20:	/* LB */		temp = RBYTE(SIMMVAL+RSVAL32); if (RTREG) RTVAL64 = (int8_t)temp;			break;
			case 0x21:	/* LH */		temp = RWORD(SIMMVAL+RSVAL32); if (RTREG) RTVAL64 = (int16_t)temp;		break;
			case 0x22:	/* LWL */		(*mips3.lwl)(op);														break;
			case 0x23:	/* LW */		temp = RLONG(SIMMVAL+RSVAL32); if (RTREG) RTVAL64 = (int32_t)temp;		break;
			case 0x24:	/* LBU */		temp = RBYTE(SIMMVAL+RSVAL32); if (RTREG) RTVAL64 = (uint8_t)temp;		break;
			case 0x25:	/* LHU */		temp = RWORD(SIMMVAL+RSVAL32); if (RTREG) RTVAL64 = (uint16_t)temp;		break;
			case 0x26:	/* LWR */		(*mips3.lwr)(op);														break;
			case 0x27:	/* LWU */		temp = RLONG(SIMMVAL+RSVAL32); if (RTREG) RTVAL64 = (uint32_t)temp;		break;
...
static void lwl_be(uint32_t op)
{
	...
	if (RTREG)
	{
		...
		{
			int shift = 8 * (offs & 3);
			RTVAL64 = (int32_t)((RTVAL32 & (0x00ffffff >> (24 - shift))) | (temp << shift));
		}
	}
}
...
static void lwr_be(uint32_t op)
{
	...
	if (RTREG)
	{
		...
		{
			int shift = 8 * (offs & 3);
			RTVAL64 = (int32_t)((RTVAL32 & (0xffffff00 << shift)) | (temp >> (24 - shift)));
		}
	}
}
...
static void lwl_le(uint32_t op)
{
	...
	if (RTREG)
	{
		...
		{
			int shift = 8 * (offs & 3);
			RTVAL64 = (int32_t)((RTVAL32 & (0x00ffffff >> shift)) | (temp << (24 - shift)));
		}
	}
}
...
static void lwr_le(uint32_t op)
{
	...
	if (RTREG)
	{
		...
		{
			int shift = 8 * (offs & 3);
			RTVAL64 = (int32_t)((RTVAL32 & (0xffffff00 << (24 - shift))) | (temp >> shift));
		}
	}
}
