...
static uint16_t  samps_per_sync;        /* Number of samples per vsync */
static uint16_t  buffer_size;           /* Actual buffer size in bytes */
static uint16_t  real_rate;             /* Actual playback rate */
static uint16_t  chip_max;              /* Desired number of chips in use */
static uint8_t   noise_lut[NOISE_LONG]; /* Noise sample lookup table */
static uint16_t  vbl_times[0x20];       /* VBL durations in samples */
static uint32_t  sync_times1[SYNCS_MAX1]; /* Samples per sync table */
static uint32_t  sync_times2[SYNCS_MAX2]; /* Samples per sync table */
static int     channel;
...
static void create_vbltimes(uint16_t * table,const uint8_t *vbl,unsigned int rate)
{
  int i;
...
static void create_noise(uint8_t *buf, const int bits, int size)
{
   static int m = 0x0011;
   int xor_val, i;
...
static int8_t apu_square(square_t *chan)
{
   int env_delay;
   int sweep_delay;
   int8_t output;
...
static int8_t apu_triangle(triangle_t *chan)
{
   int freq;
   int8_t output;
   ...
   */
   ...
   return (int8_t) chan->output_vol;
}

/* OUTPUT NOISE WAVE SAMPLE (VALUES FROM -16 to +15) */
static int8_t apu_noise(noise_t *chan)
{
   int freq, env_delay;
   uint8_t outvol;
   uint8_t output;
...
{
   chan->address = 0xC000 + (uint16_t) (chan->regs[2] << 6);
   chan->length = (uint16_t) (chan->regs[3] << 4) + 1;
...
static int8_t apu_dpcm(dpcm_t *chan)
{
   int freq, bit_pos;
...
//            chan->regs[1]--;
   ...
   return (int8_t) (chan->vol >> 1);
}

/* WRITE REGISTER VALUE */
static INLINE void apu_regwrite(int chip,int address, uint8_t value)
{
...
static INLINE uint8_t apu_read(int chip,int address)
{
  return APU[chip].regs[address];
}

/* WRITE VALUE TO TEMP REGISTRY AND QUEUE EVENT */
static INLINE void apu_write(int chip,int address, uint8_t value)
{
