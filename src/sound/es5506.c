...
static int16_t *ulaw_lookup;
static uint16_t *volume_lookup;
static uint32_t accum_mask;
...
static int compute_tables(void)
{
	...
	for (i = 0; i < (1 << ULAW_MAXBITS); i++)
	{
		...
		if (exponent == 0)
			ulaw_lookup[i] = (int16_t)mantissa >> 7;
		else
		{
			mantissa = (mantissa >> 1) | (~mantissa & 0x8000);
			ulaw_lookup[i] = (int16_t)mantissa >> (7 - exponent);
		}
	}
...
#define interpolate(sample1, sample2, accum)										\
		(sample1 * (int32_t)(0x800 - (voice->accum & 0x7ff)) + 						\
		 sample2 * (int32_t)(voice->accum & 0x7ff)) >> 11;
...
static void generate_dummy(struct ES5506Voice *voice, uint16_t *base, int32_t *lbuffer, int32_t *rbuffer, int samples)
{
	uint32_t freqcount = voice->freqcount;
	uint32_t accum = voice->accum & accum_mask;
...
static void generate_ulaw(struct ES5506Voice *voice, uint16_t *base, int32_t *lbuffer, int32_t *rbuffer, int samples)
{
	uint32_t freqcount = voice->freqcount;
	uint32_t accum = voice->accum & accum_mask;
	int32_t lvol = volume_lookup[voice->lvol >> 4];
	int32_t rvol = volume_lookup[voice->rvol >> 4];
...
reverse:
		...
		if (!(voice->control & CONTROL_DIR))
		{
			...
			while (samples--)
			{
				...
				int32_t val2 = base[((accum + (1 << 11)) & accum_mask) >> 11];
...
static void generate_pcm(struct ES5506Voice *voice, uint16_t *base, int32_t *lbuffer, int32_t *rbuffer, int samples)
{
	uint32_t freqcount = voice->freqcount;
	uint32_t accum = voice->accum & accum_mask;
	int32_t lvol = volume_lookup[voice->lvol >> 4];
...
reverse:
		...
		if (!(voice->control & CONTROL_DIR))
		{
			...
			while (samples--)
			{
				...
				int32_t val2 = (int16_t)base[((accum + (1 << 11)) & accum_mask) >> 11];
...
static void generate_samples(struct ES5506Chip *chip, int32_t *left, int32_t *right, int samples)
{
	int v;
...
static void es5506_update(int num, int16_t **buffer, int length)
{
...
int ES5506_sh_start(const struct MachineSound *msound)
{
	...
	for (i = 0; i < intf->num; i++)
	{
		...
		es5506[i].region_base[0] = intf->region0[i] ? (uint16_t *)memory_region(intf->region0[i]) : NULL;
		es5506[i].region_base[1] = intf->region1[i] ? (uint16_t *)memory_region(intf->region1[i]) : NULL;
		es5506[i].region_base[2] = intf->region2[i] ? (uint16_t *)memory_region(intf->region2[i]) : NULL;
		es5506[i].region_base[3] = intf->region3[i] ? (uint16_t *)memory_region(intf->region3[i]) : NULL;
...
static INLINE void es5506_reg_write_low(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset, uint32_t data)
{
	switch (offset)
	{
		case 0x00/8:	/* CR */
...
static INLINE void es5506_reg_write_high(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset, uint32_t data)
{
	switch (offset)
	{
		case 0x00/8:	/* CR */
...
static INLINE void es5506_reg_write_test(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset, uint32_t data)
{
	switch (offset)
	{
#ifdef LOG_COMMANDS
...
static void es5506_reg_write(struct ES5506Chip *chip, uint32_t offset, uint8_t data)
{
...
static INLINE uint32_t es5506_reg_read_low(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset)
{
	uint32_t result = 0;
...
static INLINE uint32_t es5506_reg_read_high(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset)
{
	uint32_t result = 0;
...
static INLINE uint32_t es5506_reg_read_test(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset)
{
	uint32_t result = 0;
...
static uint8_t es5506_reg_read(struct ES5506Chip *chip, uint32_t offset)
{
	struct ES5506Voice *voice = &chip->voice[chip->current_page & 0x1f];
	int shift = 8 * (offset & 3);
...
static INLINE void es5505_reg_write_low(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset, uint16_t data, uint16_t mem_mask)
{
	switch (offset)
	{
		case 0x00:	/* CR */
...
static INLINE void es5505_reg_write_high(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset, uint16_t data, uint16_t mem_mask)
{
	switch (offset)
	{
		case 0x00:	/* CR */
...
static INLINE void es5505_reg_write_test(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset, uint16_t data, uint16_t mem_mask)
{
	switch (offset)
	{
		case 0x00:	/* CH0L */
...
static void es5505_reg_write(struct ES5506Chip *chip, uint32_t offset, uint16_t data, uint16_t mem_mask)
{
	struct ES5506Voice *voice = &chip->voice[chip->current_page & 0x1f];
...
static INLINE uint16_t es5505_reg_read_low(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset)
{
	uint16_t result = 0;
...
static INLINE uint16_t es5505_reg_read_high(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset)
{
	uint16_t result = 0;
...
static INLINE uint16_t es5505_reg_read_test(struct ES5506Chip *chip, struct ES5506Voice *voice, uint32_t offset)
{
	uint16_t result = 0;
...
static uint16_t es5505_reg_read(struct ES5506Chip *chip, uint32_t offset)
{
	struct ES5506Voice *voice = &chip->voice[chip->current_page & 0x1f];
	uint16_t result = 0;
